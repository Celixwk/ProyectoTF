...
exports.obtenerEmpleadosConAreas = exports.obtenerEmpleadosNoAsignadosPorDia = exports.validarProgramacion = exports.eliminarProgramacion = exports.obtenerDetalleProgramacion = exports.obtenerNovedadesPeriodo = exports.guardarCambiosManuales = exports.regenerarDesdeFecha = exports.generarAutomatica = exports.verificarProgramacionExistente = void 0;
...
const verificarProgramacionExistente = async (req, res) => {
    try {
...
exports.verificarProgramacionExistente = verificarProgramacionExistente;
const generarAutomatica = async (req, res) => {
    ...
    try {
        ...
        if (empleadosActivos.length === 0)
            return res.status(400).json({ success: false, error: 'No hay empleados activos.' });
        ...
        turnosBD.forEach(t => {
            if (t.tipo_turno)
                tIds[t.tipo_turno] = t.id_turno;
        });
        const getIds = (codigos) => codigos.map(c => tIds[c]).filter(id => id !== undefined);
        const configReal = {};
        ...
        for (let dia = 1; dia <= diasMes; dia++) {
            ...
            const resultadoDia = await (0, capa6_integracion_1.capa6_generarProgramacionDia)(fechaProceso, {
                configuracion: configReal,
                idUsuario: id_usuario_registro ? Number(id_usuario_registro) : undefined,
                maxDiasConsecutivosArea: 3
            });
            totalAsignaciones += (_d = (_b = (_a = resultadoDia.resumen) === null || _a === void 0 ? void 0 : _a.total_asignaciones) !== null && _b !== void 0 ? _b : (_c = resultadoDia.asignaciones) === null || _c === void 0 ? void 0 : _c.length) !== null && _d !== void 0 ? _d : 0;
            const guardado = (_e = resultadoDia.guardado) !== null && _e !== void 0 ? _e : null;
            if (guardado) {
                totalGuardadas += (_f = guardado.guardadas) !== null && _f !== void 0 ? _f : 0;
                totalErrores += (_g = guardado.errores) !== null && _g !== void 0 ? _g : 0;
            }
...
exports.generarAutomatica = generarAutomatica;
const regenerarDesdeFecha = async (req, res) => {
    try {
...
exports.regenerarDesdeFecha = regenerarDesdeFecha;
const guardarCambiosManuales = async (req, res) => {
    try {
...
exports.guardarCambiosManuales = guardarCambiosManuales;
const obtenerNovedadesPeriodo = async (req, res) => {
    try {
        const { mes, anio } = req.query;
...
exports.obtenerNovedadesPeriodo = obtenerNovedadesPeriodo;
const obtenerDetalleProgramacion = async (req, res) => {
    try {
...
exports.obtenerDetalleProgramacion = obtenerDetalleProgramacion;
...
exports.eliminarProgramacion = eliminarProgramacion;
const validarProgramacion = async (req, res) => {
    try {
        const { inicio, fin } = req.query;
        if (!inicio || !fin)
        ...
        for (let i = 0; i < diasDelPeriodo; i++) {
            ...
            for (const area of areas) {
                if (area.id_area === 13)
                    continue;
                const asignados = programacionNormalizada.filter(p => p.fechaStr === fechaStr && p.id_area === area.id_area).length;
        ...
        if (programacion.length > 0) {
            ...
            for (const prog of programacion) {
                ...
                const conflicto = novedades.find(nov => nov.novedad_empleado.id_empleado === prog.id_empleado &&
...
exports.validarProgramacion = validarProgramacion;
const obtenerEmpleadosNoAsignadosPorDia = async (req, res) => {
    try {
        const { mes, anio } = req.query;
        ...
        for (let dia = 1; dia <= diasMes; dia++) {
            ...
            const asignadosEseDia = new Set(asignaciones
                .filter(a => a.fecha.toISOString().split('T')[0] === fechaKey)
                .map(a => a.id_empleado));
            resultado[fechaKey] = empleados.filter(e => {
                if (asignadosEseDia.has(e.id_empleado))
                    return false;
                const tieneNovedad = e.novedad_empleado.some(nov => nov.detalle_novedad.some(det => det.fecha.toISOString().split('T')[0] === fechaKey));
                if (tieneNovedad)
                    return false;
...
exports.obtenerEmpleadosNoAsignadosPorDia = obtenerEmpleadosNoAsignadosPorDia;
const obtenerEmpleadosConAreas = async (req, res) => {
    try {
        const empleados = await prisma.empleado.findMany({
            where: { id_estado: 1 },
            select: {
...
exports.obtenerEmpleadosConAreas = obtenerEmpleadosConAreas;
