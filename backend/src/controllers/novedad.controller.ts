...
export const novedadController = {
    ...
    async listarTipos(req: Request, res: Response) {
        try {
            const tipos = await prisma.tipoNovedad.findMany({ where: { activo: true }, orderBy: { id_novedad_tipo: 'asc' } });
    ...
    async sincronizarNovedades(req: Request, res: Response) {
        try {
            ...
            const tiposValidos = await prisma.tipoNovedad.findMany({ select: { id_novedad_tipo: true } });
            ...
            await prisma.$transaction(async (tx) => {
                for (const op of operaciones) {
                    ...
                    if (!idsValidos.has(Number(op.id_tipo))) continue;
