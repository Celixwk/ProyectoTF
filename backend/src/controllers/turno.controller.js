...
exports.asignarTurno = exports.deleteTurno = exports.updateTurno = exports.createTurno = exports.getTurnoById = exports.obtenerTurnosAsignados = exports.getTurnos = void 0;
...
const calcularDuracion = (e1, s1, e2, s2) => {
    let totalMs = 0;
    const sumarTramo = (inicio, fin) => {
    ...
    if (e1 && s1)
        totalMs += sumarTramo(e1, s1);
    if (e2 && s2)
        totalMs += sumarTramo(e2, s2);
    return Number((totalMs / (1000 * 60 * 60)).toFixed(2));
};
const getTurnos = async (req, res) => {
    try {
        const { estado } = req.query;
        const turnosRaw = await cliente_1.default.turno.findMany({
...
const obtenerTurnosAsignados = async (req, res) => {
    try {
        ...
        const turnos = await cliente_1.default.vw_turnos_asignados.findMany({
            where,
            orderBy: { fecha: 'asc' }
        });
        res.json({ success: true, data: { turnos } });
...
exports.obtenerTurnosAsignados = obtenerTurnosAsignados;
const getTurnoById = async (req, res) => {
    try {
        const { id } = req.params;
        const turnoRaw = await cliente_1.default.turno.findUnique({
...
const createTurno = async (req, res) => {
    try {
        ...
        const nuevoTurno = await cliente_1.default.turno.create({
            data: dataToSave
        });
        const respuesta = {
            ...nuevoTurno,
...
const updateTurno = async (req, res) => {
    try {
        ...
        if (hEntrada !== undefined)
            dataToUpdate.hora_entrada = hEntrada;
        if (hSalida !== undefined)
            dataToUpdate.hora_salida = hSalida;
        if (hora_entrada_2 !== undefined)
            dataToUpdate.hora_entrada_2 = convertirHora(hora_entrada_2);
        if (hora_salida_2 !== undefined)
            dataToUpdate.hora_salida_2 = convertirHora(hora_salida_2);
        if (duracion_horas) {
            ...
            const turnoActual = await cliente_1.default.turno.findUnique({ where: { id_turno: Number(id) } });
            const hE2 = dataToUpdate.hora_entrada_2 || (turnoActual === null || turnoActual === void 0 ? void 0 : turnoActual.hora_entrada_2);
            const hS2 = dataToUpdate.hora_salida_2 || (turnoActual === null || turnoActual === void 0 ? void 0 : turnoActual.hora_salida_2);
            dataToUpdate.duracion_horas = calcularDuracion(hEntrada, hSalida, hE2, hS2);
        }
        const turnoActualizado = await cliente_1.default.turno.update({
            where: { id_turno: Number(id) },
            data: dataToUpdate
        });
        const respuesta = {
...
const deleteTurno = async (req, res) => {
    try {
        const { id } = req.params;
        await cliente_1.default.turno.delete({
            where: { id_turno: Number(id) }
        });
...
const asignarTurno = async (req, res) => {
    try {
        ...
        const laborMes = await cliente_1.default.laborMes.findFirst({
            where: {
...
exports.asignarTurno = asignarTurno;
