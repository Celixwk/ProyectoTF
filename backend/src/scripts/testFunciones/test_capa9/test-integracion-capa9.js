...
async function testIntegracionCapa9() {
    ...
    const disponibles = [
        { id_empleado: 1, nombre_completo: "User 1", disponible: true },
        { id_empleado: 2, nombre_completo: "User 2", disponible: true },
        { id_empleado: 3, nombre_completo: "User 3", disponible: true }
    ];
    const fechaAyer = new Date(fecha);
    fechaAyer.setDate(fecha.getDate() - 1);
    const fechaAntier = new Date(fecha);
    fechaAntier.setDate(fecha.getDate() - 2);
    const fecha3 = new Date(fecha);
    fecha3.setDate(fecha.getDate() - 3);
    const fecha4 = new Date(fecha);
    fecha4.setDate(fecha.getDate() - 4);
    const programacion = [
    ...
    const alertas = (0, capa9_deteccionProblemas_1.capa9_detectarProblemas)(programacion, areas, disponibles, maximos, fecha, {
