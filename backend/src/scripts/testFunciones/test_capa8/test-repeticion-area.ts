...
import { capa8_verificarRepeticionArea } from "../../../services/programacion/capa8.reglasBlandas";
...
async function testRepeticionArea() {
    ...
    const historialVacio: Asignacion[] = [];

    const res1 = capa8_verificarRepeticionArea(empleado, area, fechaActual, historialVacio, maxDias);
    ...
    const fecha1 = new Date(fechaActual); fecha1.setDate(fechaActual.getDate() - 1);
    const fecha2 = new Date(fechaActual); fecha2.setDate(fechaActual.getDate() - 2);

    const historialCaso2: any[] = [
        { id_empleado: 1, id_area: 1, fecha: fecha1 },
        { id_empleado: 1, id_area: 1, fecha: fecha2 }
    ];

    const res2 = capa8_verificarRepeticionArea(empleado, area, fechaActual, historialCaso2, maxDias);
    ...
    const fecha3 = new Date(fechaActual); fecha3.setDate(fechaActual.getDate() - 3);

    const historialCaso3: any[] = [
        ...historialCaso2,
        { id_empleado: 1, id_area: 1, fecha: fecha3 }
    ];

    const res3 = capa8_verificarRepeticionArea(empleado, area, fechaActual, historialCaso3, maxDias);
    console.log(`Días consecutivos encontrados: ${res3.diasConsecutivos}`);
    console.log(`¿Viola regla? ${res3.viola ? "⚠️ SÍ" : "✅ NO"}`);
    if (res3.viola) console.log(`Mensaje: ${res3.mensaje}`);

    // CASO 4: Racha interrumpida
    console.log("\nCASO 4: Racha interrumpida (Trabajó hace 1 día, pero no hace 2)");
    const historialCaso4: any[] = [
        { id_empleado: 1, id_area: 1, fecha: fecha1 },
        // Falta fecha2
        { id_empleado: 1, id_area: 1, fecha: fecha3 }
    ];

    const res4 = capa8_verificarRepeticionArea(empleado, area, fechaActual, historialCaso4, maxDias);
