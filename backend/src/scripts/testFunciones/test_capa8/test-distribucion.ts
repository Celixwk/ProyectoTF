...
import { capa8_verificarDistribucionEquitativa } from "../../../services/programacion/capa8.reglasBlandas";
...
async function testDistribucion() {
    ...
    const historialCaso1 = Array(10).fill({ id_empleado: 50 }) as Asignacion[];

    const res1 = capa8_verificarDistribucionEquitativa(empleado, historialCaso1, promedioGlobal);
    ...
    const historialCaso2 = Array(15).fill({ id_empleado: 50 }) as Asignacion[];

    const res2 = capa8_verificarDistribucionEquitativa(empleado, historialCaso2, promedioGlobal);
    console.log(`Diferencia: ${res2.diferencia}`);
    console.log(`¿Viola regla? ${res2.viola ? "⚠️ SÍ" : "✅ NO"}`);
    if (res2.viola) console.log(`Mensaje: ${res2.mensaje}`);
    ...
    const historialCaso3 = Array(5).fill({ id_empleado: 50 }) as Asignacion[];

    const res3 = capa8_verificarDistribucionEquitativa(empleado, historialCaso3, promedioGlobal);
