...
import { capa7_verificarHorasMaximas } from "../../../services/programacion/capa7.reglasDuras";
...
async function testHorasMaximas() {
    ...
    const resultado1 = capa7_verificarHorasMaximas(turnoNormal);
    console.log(`Resultado: ${resultado1.valido ? "✅ VÁLIDO" : "❌ INVÁLIDO"}`);
    if (!resultado1.valido) console.log(`Razón: ${resultado1.razon}`);

    // CASO 2: Turno Excesivo (14 horas) - Debe RECHAZAR
    console.log("\nCASO 2: Turno de 14 horas (> 12)");
    const turnoExcesivo = { duracion_horas: 14 };
    const resultado2 = capa7_verificarHorasMaximas(turnoExcesivo);
    console.log(`Resultado: ${resultado2.valido ? "✅ VÁLIDO" : "❌ INVÁLIDO"}`);
    if (!resultado2.valido) console.log(`Razón: ${resultado2.razon}`);
    ...
    const resultado3 = capa7_verificarHorasMaximas(turnoSinInfo as any);
    console.log(`Resultado: ${resultado3.valido ? "✅ VÁLIDO" : "❌ INVÁLIDO"}`);
    if (!resultado3.valido) console.log(`Razón: ${resultado3.razon}`);

    // CASO 4: Límite personalizado (Ej: max 6 horas)
    console.log("\nCASO 4: Límite personalizado (Máx 6 horas, Turno 8 horas)");
    const resultado4 = capa7_verificarHorasMaximas(turnoNormal, 6);
    console.log(`Resultado: ${resultado4.valido ? "✅ VÁLIDO" : "❌ INVÁLIDO"}`);
    if (!resultado4.valido) console.log(`Razón: ${resultado4.razon}`);
