...
import { capa7_verificarTurnosSimultaneos } from "../../../services/programacion/capa7.reglasDuras";
...
async function testTurnosSimultaneos() {
    ...
    const inicioNoche = new Date(fechaBase); inicioNoche.setHours(20, 0, 0);
    const finNoche = new Date(fechaBase); finNoche.setHours(4, 0, 0); // Ojo: cruza día, pero para simplificar test usamos mismo día next
    finNoche.setDate(finNoche.getDate() + 1);

    const programacionCaso1: any[] = [{
    ...
    const inicioManana = new Date(fechaBase); inicioManana.setHours(8, 0, 0);
    const finManana = new Date(fechaBase); finManana.setHours(16, 0, 0);
    ...
    const res1 = capa7_verificarTurnosSimultaneos(empleado, turnoNuevo1, fechaBase, programacionCaso1);
    console.log(`Resultado: ${res1.valido ? "✅ VÁLIDO" : "❌ INVÁLIDO"}`);
    if (!res1.valido) console.log(`Razón: ${res1.razon}`);

    // CASO 2: Solapamiento Total (Mismo horario) - Debe RECHAZAR
    console.log("\nCASO 2: Turno Nuevo vs Existente (Mismo horario) - SOLAPAN");

    const programacionCaso2: any[] = [{
        id_asignacion: 2,
        id_empleado: 1,
        fecha: fechaBase,
        id_turno: 200,
    ...
    const res2 = capa7_verificarTurnosSimultaneos(empleado, turnoNuevo1, fechaBase, programacionCaso2);
    console.log(`Resultado: ${res2.valido ? "✅ VÁLIDO" : "❌ INVÁLIDO"}`);
    if (!res2.valido) console.log(`Razón: ${res2.razon}`);

    // CASO 3: Solapamiento Parcial - Debe RECHAZAR
    console.log("\nCASO 3: Solapamiento Parcial");
    console.log("Existente: 08:00 - 16:00 | Nuevo: 14:00 - 22:00");

    const inicioTarde = new Date(fechaBase); inicioTarde.setHours(14, 0, 0);
    const finTarde = new Date(fechaBase); finTarde.setHours(22, 0, 0);
    ...
    const res3 = capa7_verificarTurnosSimultaneos(empleado, turnoNuevo3, fechaBase, programacionCaso2); // Usamos el de mañana
    ...
    const finExacto = new Date(fechaBase); finExacto.setHours(12, 0, 0);
    const inicioExacto = new Date(fechaBase); inicioExacto.setHours(12, 0, 0);
    const finTarde2 = new Date(fechaBase); finTarde2.setHours(20, 0, 0);

    const programacionCaso4: any[] = [{
        id_asignacion: 4,
        id_empleado: 1,
        fecha: fechaBase,
        hora_entrada: inicioManana,
    ...
    const res4 = capa7_verificarTurnosSimultaneos(empleado, turnoNuevo4, fechaBase, programacionCaso4);
    ...
    const res5 = capa7_verificarTurnosSimultaneos(empleado, turnoNuevo5, fechaBase, programacionCaso2);
    console.log(`Resultado: ${res5.valido ? "✅ VÁLIDO" : "❌ INVÁLIDO"}`);
    if (!res5.valido) console.log(`Razón: ${res5.razon}`);

    console.log("\nCASO 6: Turno Partido (Nuevo solapa con el segundo bloque)");
    const programacionCaso6: any[] = [{
        id_asignacion: 6,
        id_empleado: 1,
        fecha: fechaBase,
        periodos: [
    ...
    const res6 = capa7_verificarTurnosSimultaneos(empleado, turnoNuevo6, fechaBase, programacionCaso6);
    console.log(`Resultado: ${res6.valido ? "✅ VÁLIDO" : "❌ INVÁLIDO"}`);
    if (!res6.valido) console.log(`Razón: ${res6.razon}`);
