...
async function main() {
    ...
    console.log("--------------------------------------------------");
    try {
        const [totalEmpleados, empleadosActivos, totalAreas, totalTurnos] = await Promise.all([
            cliente_1.default.empleado.count(),
        ...
        const muestraEmpleado = await cliente_1.default.empleado.findFirst({
