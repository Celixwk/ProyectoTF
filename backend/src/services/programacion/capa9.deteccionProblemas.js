...
exports.capa9_detectarProblemas = capa9_detectarProblemas;
const capa4_gestionHuecos_1 = require("./capa4.gestionHuecos");
const MS_POR_DIA = 86400000;
const toDateSafe = (v) => {
    const d = v instanceof Date ? v : new Date(v);
    return isNaN(d.getTime()) ? null : d;
};
const fechaSoloDiaMsUTC = (d) => {
    const dt = toDateSafe(d);
    return dt ? Date.UTC(dt.getUTCFullYear(), dt.getUTCMonth(), dt.getUTCDate()) : null;
};
function capa9_detectarProblemas(programacion, areas, empleadosDisponibles, maximosPorArea, fecha, opciones) {
    var _a;
    const alertas = [];
    const corruptas = [];
    ...
    const alertasRacha = detectarRachasUTC(programacion, (_a = opciones === null || opciones === void 0 ? void 0 : opciones.maxDiasConsecutivos) !== null && _a !== void 0 ? _a : 3, corruptas);
    alertas.push(...alertasRacha);
    if ((opciones === null || opciones === void 0 ? void 0 : opciones.descansosRequeridos) && (opciones === null || opciones === void 0 ? void 0 : opciones.empleados)) {
...
function detectarRachasUTC(programacion, max, corruptas) {
    const alertas = [];
...
function crearAlertaRacha(asig, racha, max, idEmp, idArea) {
    return {
        tipo: 'advertencia',
...
function detectarDescansosFaltantes(prog, emps, reqs, fecha) {
    const alertas = [];
    const diasMes = new Date(fecha.getUTCFullYear(), fecha.getUTCMonth() + 1, 0).getUTCDate();
    const mapeoDiasTrabajados = new Map();
    prog.forEach(a => {
    ...
    emps.forEach(e => {
        ...
        if (!req)
            return;
        const diasTrabajados = ((_a = mapeoDiasTrabajados.get(e.id_empleado)) === null || _a === void 0 ? void 0 : _a.size) || 0;
