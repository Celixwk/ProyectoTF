...
interface OpcionesAsignacion {
  ...
  maxDiasConsecutivos?: number; // Por defecto: 3
  penalizarRepeticiones?: boolean; // Por defecto: true
}


function capa5_calcularScore(
  empleado: EmpleadoOrdenado,
  idArea: number,
  ...
  const pesoClasificacion = 
      ...
      empleado.clasificacion === "flexible" ? 50 : 100; // comodín
  ...
  for (let d = 1; d <= 3; d++) {
      ...
      else break; // Si no trabajó, rompe la racha
  }
...
export function capa5_generarAsignacionesDia(
  empleadosOrdenados: EmpleadoOrdenado[], // De CAPA 1
  empleadosDisponibles: EmpleadoDisponible[], // De CAPA 2
  areasPriorizadas: AreaPriorizada[], // De CAPA 3
  maximosPorArea: Map<number, number>,
  turno: Turno,
  fecha: Date,
  opciones?: OpcionesAsignacion
      ...
      if (comodinesDisponibles.length > 0) {
          ...
          activados.forEach(act => {
              ...
              resultado.detalles.push({
                  ...
                  score: 999 // Comodín activado (score alto para indicar que fue forzado)
              });
          });
