...
exports.capa5_calcularScore = capa5_calcularScore;
exports.capa5_seleccionarEmpleadoParaArea = capa5_seleccionarEmpleadoParaArea;
exports.capa5_generarAsignacionesDia = capa5_generarAsignacionesDia;
const ID_AREA_REFUERZOS = 13;
function capa5_calcularScore(empleado, idArea, programacionHistorica, fecha, esFallback = false, penalizacion = 500) {
    const pesoClasificacion = empleado.clasificacion === "especialista" ? 0 :
        empleado.clasificacion === "flexible" ? 50 : 100;
    const totalAsignaciones = programacionHistorica.filter(p => p.id_empleado === empleado.id_empleado).length;
    const repeticionesArea = programacionHistorica.filter(p => p.id_empleado === empleado.id_empleado && p.id_area === idArea).length;
...
function capa5_seleccionarEmpleadoParaArea(area, empleadosDisponibles, turno, fecha, opciones) {
    var _a, _b;
    const empleadosYaAsignados = (opciones === null || opciones === void 0 ? void 0 : opciones.empleadosYaAsignados) || new Set();
    const programacionExistente = (opciones === null || opciones === void 0 ? void 0 : opciones.programacionExistente) || [];
    const maxDiasConsecutivos = (_a = opciones === null || opciones === void 0 ? void 0 : opciones.maxDiasConsecutivos) !== null && _a !== void 0 ? _a : 3;
    const penalizacionRefuerzo = (_b = opciones === null || opciones === void 0 ? void 0 : opciones.penalizacionRefuerzoFallback) !== null && _b !== void 0 ? _b : 500;
    const permitirFallback = (opciones === null || opciones === void 0 ? void 0 : opciones.permitirRefuerzoComoFallback) !== false;
    const poolCandidatos = empleadosDisponibles.filter(e => e.disponible && !empleadosYaAsignados.has(e.id_empleado));
    const candidatosPrimarios = poolCandidatos.filter(e => !e.areas.some(a => a.id_area === ID_AREA_REFUERZOS) &&
        (e.areas.length === 0 || e.areas.some(a => a.id_area === area.id_area)));
    const ejecutarFiltrosYScoring = (lista, esFallback) => {
        let filtrados = lista;
        if (opciones === null || opciones === void 0 ? void 0 : opciones.validarReglasFn) {
            filtrados = filtrados.filter(e => opciones.validarReglasFn(e, { id_area: area.id_area, nombre_area: area.nombre_area }, turno, fecha, programacionExistente).valido);
        }
    ...
    const areaHabilitadaParaRefuerzo = !(opciones === null || opciones === void 0 ? void 0 : opciones.areasQueUsanRefuerzo) || opciones.areasQueUsanRefuerzo.has(area.id_area);
    if (permitirFallback && areaHabilitadaParaRefuerzo) {
        const candidatosRefuerzo = poolCandidatos.filter(e => e.areas.some(a => a.id_area === ID_AREA_REFUERZOS));
...
function capa5_generarAsignacionesDia(empleadosOrdenados, empleadosDisponibles, areasPriorizadas, maximosPorArea, turno, fecha, opciones) {
    var _a, _b;
    const asignaciones = [];
    const empleadosAsignados = (_a = opciones === null || opciones === void 0 ? void 0 : opciones.empleadosYaAsignados) !== null && _a !== void 0 ? _a : new Set();
    const advertencias = [];
    const detalles = [];
    const programacionExistente = (opciones === null || opciones === void 0 ? void 0 : opciones.programacionExistente) || [];
    const cuposRestantes = (_b = opciones === null || opciones === void 0 ? void 0 : opciones.cuposRestantes) !== null && _b !== void 0 ? _b : new Map(maximosPorArea);
    ...
    for (const area of areasOrdenadas) {
        ...
        if (cuposDisponiblesArea <= 0)
            continue;
        for (let i = 0; i < cuposDisponiblesArea; i++) {
            const seleccion = capa5_seleccionarEmpleadoParaArea(area, empleadosDisponibles, turno, fecha, {
                ...opciones,
            ...
            if (opciones === null || opciones === void 0 ? void 0 : opciones.empleadosYaAsignados)
                opciones.empleadosYaAsignados.add(seleccion.empleado.id_empleado);
            detalles.push({
    ...
    let huecos = [];
    if (opciones === null || opciones === void 0 ? void 0 : opciones.detectarHuecosFn) {
